...
Microservico de Catalogo de Produtos do Continuous Testing Framework. TCC de Dorigao-LTDA.
...
|---|---|---|
| GET | `/api/catalogo` | Listar todos os produtos |
| GET | `/api/catalogo?categoria=X` | Filtrar produtos por categoria |
| GET | `/api/catalogo/{id}` | Buscar produto por UUID |
| POST | `/api/catalogo` | Criar produto |
| PUT | `/api/catalogo/{id}` | Atualizar produto |
| DELETE | `/api/catalogo/{id}` | Remover produto |

Campos do POST/PUT: `nome` (String), `descricao` (String), `preco` (BigDecimal), `categoria` (String), `quantidadeEstoque` (Integer).
...
| `OTEL_SERVICE_NAME` | `catalogo` | Nome do servico no tracing |
...
# Docker
docker build -t svc-catalogo .
docker run -p 8080:8080 svc-catalogo

# Teste rapido
curl http://localhost:8080/api/catalogo
curl -X POST http://localhost:8080/api/catalogo \
  -H "Content-Type: application/json" \
  -d '{"nome":"Produto Teste","descricao":"Descricao","preco":29.90,"categoria":"eletronicos","quantidadeEstoque":10}'
```
...
- `ingress.hosts`: `catalogo.dorigao.dev.br`
- `resources`: requests e limits de CPU/memoria
